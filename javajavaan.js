...
const dataDivisi = [
  ...
  },
  {
    ...
    icon: '',
    prokerList: ['Musyawarah Kerja', 'Upgrading Pengurus', 'Temu Wicara', 'PKKMB Prodi', 'LKMM-PD'],
...
function renderProkerPage() {
  ...
  dataDivisi.forEach(div => {
    const prokers = div.prokerList.map(item => `<li><i class="fas fa-check-circle me-2 text-primary"></i>${item}</li>`).join('');
    const nonProkers = div.nonProkerList.map(item => `<li><i class="fas fa-sync-alt me-2 text-info"></i>${item}</li>`).join('');

    const html = `
      <div class="col-md-6 mb-4 animate-item">
